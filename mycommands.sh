...
if [ "$1" = "startbot" ];then
	...
	case "${MESSAGE}" in
		...
		###
		# GROUP TOPIC EXAMPLE MESSAGES
		###

		'/send_normal_topic'*)
			# NOTICE: I have not tested it enough to be 100% certain I implemented the normal message functions around this addition properly.
			# also thread number hardcoded for now.
			threadNumber=5 # Will need to replace this with a built in logic that finds the threadNumber via the API eventually, but this works for now!
			send_normal_topic_message "${CHAT[ID]}" "${MESSAGE}" "${threadNumber}"
			;;

		'/send_html_topic'*)
			# FOR NOW HAS HARDCODED THREAD #, until it is implemented into the main codebase.
			threadNumber=3
			send_html_topic_message "${CHAT[ID]}" "${MESSAGE}" "${threadNumber}"
			;;
	esac
     }
     mycallbacks() {
